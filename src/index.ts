...
function startProgram() {
    ...
    inquirer.prompt({
        ...
        const option: string = answer.options;
        switch (option) {
...
async function addEmployee(): Promise<void> {
    try {
        ...
        const roleChoices = rows.map(({ id, title }: { id: number; title: string }) => ({
            name: title,
            value: id,
        }));

        const managerChoices = rows.map(({ id, first_name }: { id: number; first_name: string }) => ({
            name: first_name,
            value: id,
        }));

        // Prompt for employee data
        const data: NewEmployeeData = await inquirer.prompt([
            {
                type: 'input',
...
async function updateEmployeeRole(): Promise<void> {
    try {
        const { rows }: { rows: Employee[] } = await dbInstance.listAllEmployees();
        ...
        const { employeeId }: { employeeId: number } = await inquirer.prompt({
            type: 'list',
            name: 'employeeId',
        ...
        const newData: NewEmployeeData = await inquirer.prompt([
            {
                type: 'input',
                name: 'first_name',
...
async function addRole(): Promise<void> {
    try {
        const { rows }: { rows: Department[] } = await dbInstance.viewDepartments();
        const departmentChoices = rows.map(({ id, name }) => ({
            name: name,
            value: id,
        }));

        const data: RoleData = await inquirer.prompt([
            {
                type: 'input',
                name: 'titleName',
...
async function addDepartment(): Promise<void> {
    try {
        const data: DepartmentData = await inquirer.prompt([
            {
                type: 'input',
                name: 'departmentName',
